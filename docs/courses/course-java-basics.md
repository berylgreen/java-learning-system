...
- 2.2 类型转换：实现 `solution(String input)`，将输入小数字符串转为 `int` 后返回整数部分。
  * 答案: String solution(String input){\n    return String.valueOf((int)Double.parseDouble(input.trim()));\n}
...
- 2.6 while 与 do-while 循环：实现 `solution(String input)`，输入正整数 `n`，返回 `1..n` 的累加和。
  * 答案: String solution(String input){\n    int n=Integer.parseInt(input.trim()),sum=0,i=1;\n    while(i<=n){sum+=i;i++;}\n    return String.valueOf(sum);\n}
...
- 3.1 类与对象：实现 `solution(String input)`，输入 `name,color,age`，返回 `Cat{name=...,color=...,age=...}`。
  * 答案: String solution(String input){\n    String[] p=input.split(",");\n    return "Cat{name="+p[0].trim()+",color="+p[1].trim()+",age="+p[2].trim()+"}";\n}
...
- 3.2 构造器与 this 关键字：实现 `solution(String input)`，输入 `name,age`，返回 `我叫{name}，今年{age}岁`。
  * 答案: String solution(String input){\n    String[] p=input.split(",");\n    return "我叫"+p[0].trim()+"，今年"+p[1].trim()+"岁";\n}
...
- 4.1 封装：实现 `solution(String input)`，输入 `余额,操作,金额`，操作为 `deposit/withdraw`，返回操作后余额。
  * 答案: String solution(String input){\n    String[] p=input.split(",");\n    int balance=Integer.parseInt(p[0].trim()),amount=Integer.parseInt(p[2].trim());\n    String op=p[1].trim();\n    if("deposit".equals(op)) balance+=amount;\n    else if("withdraw".equals(op)) balance-=amount;\n    return String.valueOf(balance);\n}
...
- 4.2 继承：实现 `solution(String input)`，输入 `cat|dog,名字`，返回该动物的特有行为描述。
  * 答案: String solution(String input){\n    String[] p=input.split(",");\n    String type=p[0].trim(),name=p[1].trim();\n    if("cat".equals(type)) return name+":喵喵叫";\n    if("dog".equals(type)) return name+":汪汪叫";\n    return "";\n}
...
- 4.3 方法重写与多态：实现 `solution(String input)`，输入 `circle,r` 或 `rect,w,h`，返回面积（保留两位小数）。
  * 答案: String solution(String input){\n    String[] p=input.split(",");\n    double area;\n    if("circle".equals(p[0].trim())){\n        double r=Double.parseDouble(p[1].trim());\n        area=Math.PI*r*r;\n    }else{\n        double w=Double.parseDouble(p[1].trim()),h=Double.parseDouble(p[2].trim());\n        area=w*h;\n    }\n    return String.format("%.2f", area);\n}
...
- 4.4 toString 与 equals 方法：实现 `solution(String input)`，输入 `name,age|name,age`，返回两个对象内容是否相等。
  * 答案: String solution(String input){\n    String[] pair=input.split("\\|");\n    String[] a=pair[0].split(","),b=pair[1].split(",");\n    return String.valueOf(a[0].trim().equals(b[0].trim())&&a[1].trim().equals(b[1].trim()));\n}
...
- 4.5 final 关键字：实现 `solution(String input)`，输入半径，使用常量 `PI=3.14159` 计算圆面积并返回。
  * 答案: String solution(String input){\n    final double PI=3.14159;\n    double r=Double.parseDouble(input.trim());\n    return String.format("%.2f", PI*r*r);\n}
  * 提示: 使用固定常量 `PI=3.14159`，按 `PI * r * r` 计算面积，并保留两位小数；重点是常量值固定不变且格式统一。
  * 公开 | 输入: 2 | 输出: 12.57
  * 隐藏 | 输入: 3 | 输出: 28.27
- 5.1 抽象类：实现 `solution(String input)`，输入 `car|boat,brand`，返回对应运行描述。
  * 答案: String solution(String input){\n    String[] p=input.split(",");\n    String type=p[0].trim(),brand=p[1].trim();\n    if("car".equals(type)) return brand+" 在陆地行驶";\n    if("boat".equals(type)) return brand+" 在水上航行";\n    return "";\n}
...
- 5.2 接口：实现 `solution(String input)`，输入 `fly|swim,name`，返回行为描述。
  * 答案: String solution(String input){\n    String[] p=input.split(",");\n    String action=p[0].trim(),name=p[1].trim();\n    if("fly".equals(action)) return name+" 正在飞行";\n    if("swim".equals(action)) return name+" 正在游泳";\n    return "";\n}
...
- 5.4 Lambda 表达式：实现 `solution(String input)`，输入 `a,b,op`，`op` 为 `add/sub/mul`，返回运算结果。
  * 答案: String solution(String input){\n    String[] p=input.split(",");\n    int a=Integer.parseInt(p[0].trim()),b=Integer.parseInt(p[1].trim());\n    String op=p[2].trim();\n    if("add".equals(op)) return String.valueOf(a+b);\n    if("sub".equals(op)) return String.valueOf(a-b);\n    if("mul".equals(op)) return String.valueOf(a*b);\n    return "";\n}
  * 提示: 拆分后将前两个字段转为整数，根据 `op` 分支计算：`add` 加法、`sub` 减法、`mul` 乘法；返回纯数字字符串。
  * 公开 | 输入: 7,3,add | 输出: 10
  * 隐藏 | 输入: 7,3,mul | 输出: 21
- 5.5 函数式接口与方法引用：实现 `solution(String input)`，输入逗号分隔姓名列表，返回姓“张”的人数。
  * 答案: String solution(String input){\n    String[] p=input.split(",");\n    int c=0;\n    for(String n:p){ if(n.trim().startsWith("张")) c++; }\n    return String.valueOf(c);\n}
...
- 6.1 异常概述：实现 `solution(String input)`，输入除数，返回 `10/除数`；若除数为 0 返回 `ArithmeticException`。
  * 答案: String solution(String input){\n    int d=Integer.parseInt(input.trim());\n    if(d==0) return "ArithmeticException";\n    return String.valueOf(10/d);\n}
...
- 6.2 try-catch-finally：实现 `solution(String input)`，输入整数，计算 `100/input`，异常时返回 `ERROR`，结果后拼接 `|FINALLY`。
  * 答案: String solution(String input){\n    String r;\n    try{ r=String.valueOf(100/Integer.parseInt(input.trim())); }catch(Exception e){ r="ERROR"; }\n    return r+"|FINALLY";\n}
...
- 6.6 Math 与 Random：实现 `solution(String input)`，输入整数 `n`，返回 `abs(n),sqrt(abs(n))`（平方根保留两位小数）。
  * 答案: String solution(String input){\n    int n=Integer.parseInt(input.trim());\n    int abs=Math.abs(n);\n    return abs+","+String.format("%.2f", Math.sqrt(abs));\n}
...
- 7.1 集合概述与 ArrayList：实现 `solution(String input)`，输入逗号分隔元素，返回列表长度与是否包含 `Java`。
  * 答案: String solution(String input){\n    String[] p=input.split(",");\n    java.util.List<String> list=new java.util.ArrayList<>();\n    for(String s:p) list.add(s.trim());\n    return list.size()+"|"+list.contains("Java");\n}
...
- 7.3 HashSet 与 TreeSet：实现 `solution(String input)`，输入逗号分隔整数，去重并升序返回。
  * 答案: String solution(String input){\n    String[] p=input.split(",");\n    java.util.TreeSet<Integer> set=new java.util.TreeSet<>();\n    for(String s:p) set.add(Integer.parseInt(s.trim()));\n    StringBuilder sb=new StringBuilder();\n    for(Integer v:set){ if(sb.length()>0) sb.append(","); sb.append(v);}\n    return sb.toString();\n}
...
- 7.4 HashMap：实现 `solution(String input)`，输入 `name=score;...`，返回分数最高的 `name`。
  * 答案: String solution(String input){\n    String[] pairs=input.split(";");\n    String bestName="";\n    int best=Integer.MIN_VALUE;\n    for(String pair:pairs){ String[] kv=pair.split("="); String name=kv[0].trim(); int score=Integer.parseInt(kv[1].trim()); if(score>best){ best=score; bestName=name; } }\n    return bestName;\n}
...
- 7.6 Stream API：实现 `solution(String input)`，输入逗号分隔整数，返回去重后偶数升序结果。
  * 答案: String solution(String input){\n    String[] p=input.split(",");\n    java.util.TreeSet<Integer> set=new java.util.TreeSet<>();\n    for(String s:p){ int v=Integer.parseInt(s.trim()); if(v%2==0) set.add(v);}\n    if(set.isEmpty()) return "(空)";\n    StringBuilder sb=new StringBuilder();\n    for(Integer v:set){ if(sb.length()>0) sb.append(","); sb.append(v);}\n    return sb.toString();\n}
...
- 8.2 字节流 InputStream 与 OutputStream：实现 `solution(String input)`，返回输入字符串 UTF-8 字节长度。
  * 答案: String solution(String input){\n    return String.valueOf(input.getBytes(java.nio.charset.StandardCharsets.UTF_8).length);\n}
...
- 9.3 线程同步：实现 `solution(String input)`，输入票数与窗口数 `tickets,workers`，返回最终剩余票数。
  * 答案: String solution(String input){\n    String[] p=input.split(",");\n    int t=Integer.parseInt(p[0].trim()),w=Integer.parseInt(p[1].trim());\n    return String.valueOf(Math.max(t-w,0));\n}
...
- 9.4 线程池：实现 `solution(String input)`，输入 `tasks,poolSize`，返回完成全部任务所需批次数。
  * 答案: String solution(String input){\n    String[] p=input.split(",");\n    int t=Integer.parseInt(p[0].trim()),s=Integer.parseInt(p[1].trim());\n    return String.valueOf((t+s-1)/s);\n}
