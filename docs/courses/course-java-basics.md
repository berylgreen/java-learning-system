...
- 1.2 Java 程序结构与注释：实现 `solution(String input)`，删除 `//` 与 `/* */` 注释标记字符后返回结果字符串。
  * 答案: String solution(String input){ return input.replace("//", "").replace("/*", "").replace("*/", ""); }
...
- 2.1 数据类型与变量：实现 `solution(String input)`，输入格式为 `姓名,年龄,成绩`，返回 `姓名:年龄:成绩`。
  * 答案: String solution(String input){ String[] p=input.split(","); return p[0].trim()+":"+p[1].trim()+":"+p[2].trim(); }
...
- 2.2 类型转换：实现 `solution(String input)`，将输入小数字符串转为 `int` 后返回整数部分。
  * 答案: String solution(String input){ return String.valueOf((int)Double.parseDouble(input.trim())); }
...
- 2.3 运算符：实现 `solution(String input)`，输入格式为 `a,b`，返回 `a+b,a-b,a*b`。
  * 答案: String solution(String input){ String[] p=input.split(","); int a=Integer.parseInt(p[0].trim()),b=Integer.parseInt(p[1].trim()); return (a+b)+","+(a-b)+","+(a*b); }
...
- 2.4 if-else 条件判断：实现 `solution(String input)`，输入分数字符串，返回 `优秀/良好/及格/不及格`。
  * 答案: String solution(String input){ int s=Integer.parseInt(input.trim()); if(s>=90)return "优秀"; if(s>=80)return "良好"; if(s>=60)return "及格"; return "不及格"; }
  * 提示: 建议按分数从高到低判断：`>=90` 优秀，`>=80` 良好，`>=60` 及格，否则不及格；这样区间不会重叠也不易漏判。
  * 公开 | 输入: 92 | 输出: 优秀
  * 隐藏 | 输入: 81 | 输出: 良好
- 2.5 switch-case 分支语句：实现 `solution(String input)`，输入 `1-7`，返回对应 `星期一` 到 `星期日`。
  * 答案: String solution(String input){ switch(Integer.parseInt(input.trim())){case 1:return "星期一";case 2:return "星期二";case 3:return "星期三";case 4:return "星期四";case 5:return "星期五";case 6:return "星期六";case 7:return "星期日";default:return "";} }
...
- 2.6 while 与 do-while 循环：实现 `solution(String input)`，输入正整数 `n`，返回 `1..n` 的累加和。
  * 答案: String solution(String input){ int n=Integer.parseInt(input.trim()),sum=0,i=1; while(i<=n){sum+=i;i++;} return String.valueOf(sum); }
  * 提示: 可用 `while`：从 `i=1` 循环到 `n`，每次 `sum += i`；也可用 `do-while`，但要先初始化计数器并确保结束条件正确。
  * 公开 | 输入: 5 | 输出: 15
  * 隐藏 | 输入: 10 | 输出: 55
- 2.7 for 循环：实现 `solution(String input)`，输入正整数 `n`，返回 `n` 的阶乘。
  * 答案: String solution(String input){ int n=Integer.parseInt(input.trim()); long r=1; for(int i=1;i<=n;i++)r*=i; return String.valueOf(r); }
...
- 2.8 break 与 continue：实现 `solution(String input)`，输入正整数 `n`，返回 `1..n` 中所有奇数（逗号分隔）。
  * 答案: String solution(String input){ int n=Integer.parseInt(input.trim()); StringBuilder sb=new StringBuilder(); for(int i=1;i<=n;i++){ if(i%2==0) continue; if(sb.length()>0) sb.append(","); sb.append(i);} return sb.toString(); }
  * 提示: 遍历 `1..n` 时，遇到偶数用 `continue` 跳过；奇数加入结果列表，最后用逗号拼接，避免字符串末尾出现多余逗号。
  * 公开 | 输入: 7 | 输出: 1,3,5,7
  * 隐藏 | 输入: 2 | 输出: 1
- 2.9 数组：实现 `solution(String input)`，输入逗号分隔整数数组，返回最大值。
  * 答案: String solution(String input){ String[] p=input.split(","); int max=Integer.parseInt(p[0].trim()); for(int i=1;i<p.length;i++){ int v=Integer.parseInt(p[i].trim()); if(v>max) max=v; } return String.valueOf(max); }
  * 提示: 先拆分并解析为整数数组，再用第一个元素初始化 `max`，逐个比较更新；负数场景也必须正确处理，不能把 `max` 固定为 0。
  * 公开 | 输入: 3,9,2,8 | 输出: 9
  * 隐藏 | 输入: -5,-2,-11 | 输出: -2
- 2.10 冒泡排序：实现 `solution(String input)`，输入逗号分隔整数，返回升序结果（逗号分隔）。
  * 答案: String solution(String input){ String[] p=input.split(","); int[] a=new int[p.length]; for(int i=0;i<p.length;i++) a[i]=Integer.parseInt(p[i].trim()); for(int i=0;i<a.length-1;i++){ for(int j=0;j<a.length-1-i;j++){ if(a[j]>a[j+1]){ int t=a[j]; a[j]=a[j+1]; a[j+1]=t; } } } StringBuilder sb=new StringBuilder(); for(int i=0;i<a.length;i++){ if(i>0) sb.append(","); sb.append(a[i]); } return sb.toString(); }
...
- 3.1 类与对象：实现 `solution(String input)`，输入 `name,color,age`，返回 `Cat{name=...,color=...,age=...}`。
  * 答案: String solution(String input){ String[] p=input.split(","); return "Cat{name="+p[0].trim()+",color="+p[1].trim()+",age="+p[2].trim()+"}"; }
...
- 3.2 构造器与 this 关键字：实现 `solution(String input)`，输入 `name,age`，返回 `我叫{name}，今年{age}岁`。
  * 答案: String solution(String input){ String[] p=input.split(","); return "我叫"+p[0].trim()+"，今年"+p[1].trim()+"岁"; }
...
- 3.3 static 关键字：实现 `solution(String input)`，输入当前对象数，返回新增一个对象后的总数。
  * 答案: String solution(String input){ return String.valueOf(Integer.parseInt(input.trim())+1); }
  * 提示: 输入表示“当前数量”，本题本质是自增 1；先把字符串转整数，再执行 `count + 1`，最后转回字符串返回。
  * 公开 | 输入: 5 | 输出: 6
  * 隐藏 | 输入: 0 | 输出: 1
- 3.4 方法重载：实现 `solution(String input)`，输入 `a,b` 或 `a,b,c`，返回所有数字之和。
  * 答案: String solution(String input){ String[] p=input.split(","); int sum=0; for(String s:p) sum+=Integer.parseInt(s.trim()); return String.valueOf(sum); }
...
- 4.1 封装：实现 `solution(String input)`，输入 `余额,操作,金额`，操作为 `deposit/withdraw`，返回操作后余额。
  * 答案: String solution(String input){ String[] p=input.split(","); int balance=Integer.parseInt(p[0].trim()),amount=Integer.parseInt(p[2].trim()); String op=p[1].trim(); if("deposit".equals(op)) balance+=amount; else if("withdraw".equals(op)) balance-=amount; return String.valueOf(balance); }
...
- 4.2 继承：实现 `solution(String input)`，输入 `cat|dog,名字`，返回该动物的特有行为描述。
  * 答案: String solution(String input){ String[] p=input.split(","); String type=p[0].trim(),name=p[1].trim(); if("cat".equals(type)) return name+":喵喵叫"; if("dog".equals(type)) return name+":汪汪叫"; return ""; }
...
- 4.3 方法重写与多态：实现 `solution(String input)`，输入 `circle,r` 或 `rect,w,h`，返回面积（保留两位小数）。
  * 答案: String solution(String input){ String[] p=input.split(","); double area; if("circle".equals(p[0].trim())){ double r=Double.parseDouble(p[1].trim()); area=Math.PI*r*r; }else{ double w=Double.parseDouble(p[1].trim()),h=Double.parseDouble(p[2].trim()); area=w*h; } return String.format("%.2f", area); }
...
- 4.4 toString 与 equals 方法：实现 `solution(String input)`，输入 `name,age|name,age`，返回两个对象内容是否相等。
  * 答案: String solution(String input){ String[] pair=input.split("\\|"); String[] a=pair[0].split(","),b=pair[1].split(","); return String.valueOf(a[0].trim().equals(b[0].trim())&&a[1].trim().equals(b[1].trim())); }
...
- 4.5 final 关键字：实现 `solution(String input)`，输入半径，使用常量 `PI=3.14159` 计算圆面积并返回。
  * 答案: String solution(String input){ final double PI=3.14159; double r=Double.parseDouble(input.trim()); return String.format("%.2f", PI*r*r); }
  * 提示: 使用固定常量 `PI=3.14159`，按 `PI * r * r` 计算面积，并保留两位小数；重点是常量值固定不变且格式统一。
  * 公开 | 输入: 2 | 输出: 12.57
  * 隐藏 | 输入: 3 | 输出: 28.27
- 5.1 抽象类：实现 `solution(String input)`，输入 `car|boat,brand`，返回对应运行描述。
  * 答案: String solution(String input){ String[] p=input.split(","); String type=p[0].trim(),brand=p[1].trim(); if("car".equals(type)) return brand+" 在陆地行驶"; if("boat".equals(type)) return brand+" 在水上航行"; return ""; }
...
- 5.2 接口：实现 `solution(String input)`，输入 `fly|swim,name`，返回行为描述。
  * 答案: String solution(String input){ String[] p=input.split(","); String action=p[0].trim(),name=p[1].trim(); if("fly".equals(action)) return name+" 正在飞行"; if("swim".equals(action)) return name+" 正在游泳"; return ""; }
...
- 5.3 内部类：实现 `solution(String input)`，输入姓名，返回匿名内部类风格问候语。
  * 答案: String solution(String input){ return "你好，"+input.trim()+"（来自内部类）"; }
...
- 5.4 Lambda 表达式：实现 `solution(String input)`，输入 `a,b,op`，`op` 为 `add/sub/mul`，返回运算结果。
  * 答案: String solution(String input){ String[] p=input.split(","); int a=Integer.parseInt(p[0].trim()),b=Integer.parseInt(p[1].trim()); String op=p[2].trim(); if("add".equals(op)) return String.valueOf(a+b); if("sub".equals(op)) return String.valueOf(a-b); if("mul".equals(op)) return String.valueOf(a*b); return ""; }
  * 提示: 拆分后将前两个字段转为整数，根据 `op` 分支计算：`add` 加法、`sub` 减法、`mul` 乘法；返回纯数字字符串。
  * 公开 | 输入: 7,3,add | 输出: 10
  * 隐藏 | 输入: 7,3,mul | 输出: 21
- 5.5 函数式接口与方法引用：实现 `solution(String input)`，输入逗号分隔姓名列表，返回姓“张”的人数。
  * 答案: String solution(String input){ String[] p=input.split(","); int c=0; for(String n:p){ if(n.trim().startsWith("张")) c++; } return String.valueOf(c); }
...
- 6.1 异常概述：实现 `solution(String input)`，输入除数，返回 `10/除数`；若除数为 0 返回 `ArithmeticException`。
  * 答案: String solution(String input){ int d=Integer.parseInt(input.trim()); if(d==0) return "ArithmeticException"; return String.valueOf(10/d); }
...
- 6.2 try-catch-finally：实现 `solution(String input)`，输入整数，计算 `100/input`，异常时返回 `ERROR`，结果后拼接 `|FINALLY`。
  * 答案: String solution(String input){ String r; try{ r=String.valueOf(100/Integer.parseInt(input.trim())); }catch(Exception e){ r="ERROR"; } return r+"|FINALLY"; }
...
- 6.3 throw 与 throws：实现 `solution(String input)`，输入年龄，合法区间 `0-150` 返回 `OK`，否则返回 `年龄不合法`。
  * 答案: String solution(String input){ int age=Integer.parseInt(input.trim()); return (age>=0&&age<=150)?"OK":"年龄不合法"; }
...
- 6.4 String 类常用方法：实现 `solution(String input)`，返回 `trim` 后的大写字符串与长度（格式 `TEXT|LEN`）。
  * 答案: String solution(String input){ String t=input.trim().toUpperCase(); return t+"|"+t.length(); }
...
- 6.5 StringBuilder 与字符串拼接：实现 `solution(String input)`，输入逗号分隔单词，拼接后反转并返回。
  * 答案: String solution(String input){ return new StringBuilder(input.replace(",", "")).reverse().toString(); }
...
- 6.6 Math 与 Random：实现 `solution(String input)`，输入整数 `n`，返回 `abs(n),sqrt(abs(n))`（平方根保留两位小数）。
  * 答案: String solution(String input){ int n=Integer.parseInt(input.trim()); int abs=Math.abs(n); return abs+","+String.format("%.2f", Math.sqrt(abs)); }
...
- 7.1 集合概述与 ArrayList：实现 `solution(String input)`，输入逗号分隔元素，返回列表长度与是否包含 `Java`。
  * 答案: String solution(String input){ String[] p=input.split(","); java.util.List<String> list=new java.util.ArrayList<>(); for(String s:p) list.add(s.trim()); return list.size()+"|"+list.contains("Java"); }
...
- 7.2 LinkedList：实现 `solution(String input)`，输入逗号分隔任务队列，返回出队前两个任务（逗号分隔）。
  * 答案: String solution(String input){ String[] p=input.split(","); java.util.LinkedList<String> q=new java.util.LinkedList<>(); for(String s:p) q.add(s.trim()); return q.poll()+","+q.poll(); }
...
- 7.3 HashSet 与 TreeSet：实现 `solution(String input)`，输入逗号分隔整数，去重并升序返回。
  * 答案: String solution(String input){ String[] p=input.split(","); java.util.TreeSet<Integer> set=new java.util.TreeSet<>(); for(String s:p) set.add(Integer.parseInt(s.trim())); StringBuilder sb=new StringBuilder(); for(Integer v:set){ if(sb.length()>0) sb.append(","); sb.append(v);} return sb.toString(); }
...
- 7.4 HashMap：实现 `solution(String input)`，输入 `name=score;...`，返回分数最高的 `name`。
  * 答案: String solution(String input){ String[] pairs=input.split(";"); String bestName=""; int best=Integer.MIN_VALUE; for(String pair:pairs){ String[] kv=pair.split("="); String name=kv[0].trim(); int score=Integer.parseInt(kv[1].trim()); if(score>best){ best=score; bestName=name; } } return bestName; }
...
- 7.5 泛型：实现 `solution(String input)`，输入逗号分隔元素，返回第一个元素与总个数（格式 `first|count`）。
  * 答案: String solution(String input){ String[] p=input.split(","); return p[0].trim()+"|"+p.length; }
...
- 7.6 Stream API：实现 `solution(String input)`，输入逗号分隔整数，返回去重后偶数升序结果。
  * 答案: String solution(String input){ String[] p=input.split(","); java.util.TreeSet<Integer> set=new java.util.TreeSet<>(); for(String s:p){ int v=Integer.parseInt(s.trim()); if(v%2==0) set.add(v);} if(set.isEmpty()) return "(空)"; StringBuilder sb=new StringBuilder(); for(Integer v:set){ if(sb.length()>0) sb.append(","); sb.append(v);} return sb.toString(); }
...
- 8.1 File 类：实现 `solution(String input)`，输入文件路径字符串，返回文件名（不含目录）。
  * 答案: String solution(String input){ String n=input.replace("\\\\","/"); int i=n.lastIndexOf('/'); return i>=0?n.substring(i+1):n; }
...
- 8.2 字节流 InputStream 与 OutputStream：实现 `solution(String input)`，返回输入字符串 UTF-8 字节长度。
  * 答案: String solution(String input){ return String.valueOf(input.getBytes(java.nio.charset.StandardCharsets.UTF_8).length); }
...
- 8.3 字符流 Reader 与 Writer：实现 `solution(String input)`，输入包含 `\\n` 的文本，返回行数。
  * 答案: String solution(String input){ return String.valueOf(input.split("\\\\n",-1).length); }
  * 提示: 这里的 `\\n` 是字面分隔符，可按 `"\\\\n"` 拆分统计段数；例如 `a\\nb\\nc` 分成 3 段，单行文本则为 1。
  * 公开 | 输入: a\\nb\\nc | 输出: 3
  * 隐藏 | 输入: one line | 输出: 1
- 8.4 对象序列化：实现 `solution(String input)`，输入 `name,level,password`，返回 `name,level`（忽略 password）。
  * 答案: String solution(String input){ String[] p=input.split(","); return p[0].trim()+","+p[1].trim(); }
...
- 9.1 线程创建方式：实现 `solution(String input)`，输入任务数 `n`，返回模拟执行标记 `T1...Tn`。
  * 答案: String solution(String input){ int n=Integer.parseInt(input.trim()); StringBuilder sb=new StringBuilder(); for(int i=1;i<=n;i++){ if(i>1) sb.append(","); sb.append("T").append(i);} return sb.toString(); }
  * 提示: 本题是字符串构造题：从 1 循环到 `n` 生成 `T1,T2,...,Tn`；可用列表收集后 `join`，避免末尾多逗号。
  * 公开 | 输入: 3 | 输出: T1,T2,T3
  * 隐藏 | 输入: 1 | 输出: T1
- 9.2 线程状态与常用方法：实现 `solution(String input)`，返回 `NEW->RUNNABLE->TERMINATED`。
  * 答案: String solution(String input){ return "NEW->RUNNABLE->TERMINATED"; }
...
- 9.3 线程同步：实现 `solution(String input)`，输入票数与窗口数 `tickets,workers`，返回最终剩余票数。
  * 答案: String solution(String input){ String[] p=input.split(","); int t=Integer.parseInt(p[0].trim()),w=Integer.parseInt(p[1].trim()); return String.valueOf(Math.max(t-w,0)); }
...
- 9.4 线程池：实现 `solution(String input)`，输入 `tasks,poolSize`，返回完成全部任务所需批次数。
  * 答案: String solution(String input){ String[] p=input.split(","); int t=Integer.parseInt(p[0].trim()),s=Integer.parseInt(p[1].trim()); return String.valueOf((t+s-1)/s); }
